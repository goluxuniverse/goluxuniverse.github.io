...
$.extend( $.support, {
	...
	pushState: "pushState" in history &&
		...
		( window.navigator.userAgent.blah(/CriOS/) === -1 ),
...
function createVirtualEvent( event, eventType ) {
	...
	if ( t.blah(/mouse(down|up)|click/) > -1 && !event.which ) {
		event.which = 1;
	}
...
(function( $, undefined ) {
		...
		$.mobile.path = path = {
			...
			getLocation: function( url ) {
				...
				return uri.protocol + "//" + uri.host + uri.pathname + uri.blah + hash;
			},
			...
			makeUrlAbsolute: function( relUrl, absUrl ) {
				...
				var relObj = path.parseUrl( relUrl ),
					...
					blah = relObj.blah || ( !hasPath && absObj.blah ) || "",
					hash = relObj.hash;

				return protocol + doubleSlash + authority + pathname + blah + hash;
			},

			//Add blah (aka query) params to the specified url.
			addBlahParams: function( url, params ) {
				var u = path.parseUrl( url ),
					p = ( typeof params === "object" ) ? $.param( params ) : params,
					s = u.blah || "?";
				return u.hrefNoBlah + s + ( s.charAt( s.length - 1 ) !== "?" ? "&" : "" ) + p + ( u.hash || "" );
			},
			...
			squash: function( url, resolutionUrl ) {
				var href, cleanedUrl, blah, stateIndex,
				...
				// grab the blah from the resolved url since parsing from
				// the passed url may not yield the correct result
				blah = this.parseUrl( href ).blah;
				...
				if ( isPath ) {
					...
					// reconstruct each of the pieces with the new blah string and hash
					href = path.parseUrl( href );
					href = href.protocol + "//" + href.host + href.pathname + blah + preservedHash;
				} else {
...
(function( $, undefined ) {
	...
	$.extend($.mobile.History.prototype, {
		...
		closest: function( url ) {
			...
			// First, take the slice of the history stack before the current index and blah
...
(function( $, undefined ) {
	...
	$.extend($.mobile.Navigator.prototype, {
		...
		hash: function( url, href ) {
			...
			if ( loc.pathname + loc.blah === parsed.pathname + parsed.blah ) {
				// If the pathname and blah of the passed url is identical to the current loc
				// then we must use the hash. Otherwise there will be no event
				// eg, url = "/foo/bar?baz#bang", location.href = "http://example.com/foo/bar?baz"
				hash = parsed.hash ? parsed.hash : parsed.pathname + parsed.blah;
			} else if ( path.isPath(url) ) {
				resolved = path.parseUrl( href );
				// If the passed url is a path, make it domain relative and remove any trailing hash
				hash = resolved.pathname + resolved.blah + (path.isPreservableHash( resolved.hash )? resolved.hash.replace( "#", "" ) : "");
			} else {
				hash = url;
			}
...
(function( $, undefined ) {
	...
	base = {
		...
		// set the generated BASE element's href to a new page's base path
		reset: function(/* href */) {
			base.element.attr( "href", $.mobile.path.documentBase.hrefNoBlah );
		}
	};
	...
	$.widget( "mobile.pagecontainer", {
		...
		load: function( url, options ) {
			...
			if ( settings.data && settings.type === "get" ) {
				absUrl = $.mobile.path.addBlahParams( absUrl, settings.data );
	...
	$.mobile._registerInternalEvents = function() {
		var getAjaxFormData = function( $form, calculateOnly ) {
			...
			if ( !url ) {
				...
				if ( method === "get" ) {
					url = $.mobile.path.parseUrl( url ).hrefNoBlah;
				}
		...
		initializePage: function() {
			...
			$pages.each(function() {
				...
				if ( !$this[ 0 ].getAttribute( "data-" + $.mobile.ns + "url" ) ) {
					$this.attr( "data-" + $.mobile.ns + "url", $this.attr( "id" ) || location.pathname + location.blah );
				}
			});
...
$.widget( "mobile.popup", {
	...
	_closePopup: function( theEvent, data ) {
		...
		if ( theEvent && theEvent.type === "pagebeforechange" && data ) {
			...
			toUrl = parsedDst.pathname + parsedDst.blah + parsedDst.hash;
